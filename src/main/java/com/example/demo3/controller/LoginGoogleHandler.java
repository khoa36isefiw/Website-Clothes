...
public class LoginGoogleHandler extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        User user1 = new User();
        if (ad.findAccountByUsername(user.getEmail().toString())==null)
        {
            account.setUsername(user.getEmail());
            account.setPassword(user.getId());
            AccountDAO.insert(account);
            user1.setAccount(account);
            user1.setEmail(user.getEmail());
            user1.setFirstName(user.getFamily_name());
            user1.setLastName(user.getGiven_name());
            UserDAO.insert(user1);
        }
        else{
            user1 = UserDAO.findUserByUsername(user.getEmail().toString());
        }
